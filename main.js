...
export const barcodeSplits = {
    ['GTIN-12']: /(\d)(\d{5})(\d{5})(\d)/,
    ['GTIN-13']: /(\d)(\d{6})(\d{6})/,
    ['GTIN-14']: /(\d)(\d{2})(\d{5})(\d{5})(\d)/,
};
export const couldBeGTIN = (gtin) => /^[0-9 ]+$/.test(gtin);
export function trimUPC(digits) {
    return digits.trim().replace(/\D/g, '');
}
export function parseCheckDigit(str, format = 'GTIN-12') {
    const digitSum = [0, 0];
...
export function parseGTINFormat(str) {
...
export function formatUPC(upc, raw = false) {
    if (!couldBeGTIN(upc)) {
        return upc;
    }
